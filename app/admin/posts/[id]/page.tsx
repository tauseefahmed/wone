    ...
    const fetchPost = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setSlugDirty(postData.slug !== createSlug(postData.title))
          ...
          setShowAuthor(!!postData.showAuthor)
          setSelectedCategories(postData.categories.map((c: any) => c.category.id))
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white shadow px-4 py-5 sm:rounded-lg sm:p-6">
          <div className="md:grid md:grid-cols-3 md:gap-6">
            <div className="md:col-span-1">
              ...
              <p className="mt-1 text-sm text-gray-500">Toggle additional sections for this post.</p>
            </div>
            <div className="mt-5 md:mt-0 md:col-span-2">
              <div className="space-y-4">
                <label className="flex items-start gap-3">
                  <input type="checkbox" className="mt-1 h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" checked={showAuthor} onChange={(e) => setShowAuthor(e.target.checked)} />
                  <span>
                    <span className="block text-sm font-medium text-gray-900">Show About Author section</span>
                    <span className="block text-sm text-gray-500">Displays the author card below the content if enabled.</span>
                  </span>
                </label>
              </div>
            </div>
        ...
        <div className="flex justify-end space-x-3">
          <button
            ...
            className="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 inline-flex items-center gap-2"
          >
            <X className="w-4 h-4" />
            Cancel
          </button>
          <button
            type="submit"
            disabled={loading}
            className="inline-flex justify-center items-center gap-2 py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50"
          >
